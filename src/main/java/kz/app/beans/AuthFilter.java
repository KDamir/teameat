...
public class AuthFilter implements Filter {
        ...
        try {
            ...
            if((authBean == null || !authBean.logged) && !requestURI.contains("/loginForm.xhtml") && !requestURI.contains("/javax.faces.resource/")) {
                if(!"XMLHttpRequest".equals(req.getHeader("X-Requested-With"))) {
                    ...
                    resxml.append("<?xml version=\"1.0\" encoding=\"UTF-8\"?><partial-response><redirect url=\"");
                    resxml.append(req.getHeader("Referer"));
                    resxml.append("\"/></partial-response>");

                    ((HttpServletRequest)request).getSession().setAttribute("reason", "sessionexpired");

                    res.getWriter().print(resxml.toString());
                    res.flushBuffer();
                }
            }
    ...
    public void init(FilterConfig filterConfig) {        
        ...
        if (filterConfig != null) {
            ...
            authValue = filterConfig.getInitParameter("authValue");
        }
    }
